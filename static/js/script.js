...
function setupFormValidations() {
    ...
    if (ageInput) {
        ageInput.addEventListener('change', function() {
            ...
            if (age < 60 || age > 120) {
                this.setCustomValidity(getI18nText('age_validation', 'Age must be between 60 and 120 years for elderly care monitoring.'));
            } else {
...
function updatePasswordStrengthIndicator(strength) {
    ...
    const strengths = [
        getI18nText('very_weak', 'Very Weak'),
        getI18nText('weak', 'Weak'),
        getI18nText('fair', 'Fair'),
        getI18nText('good', 'Good'),
        getI18nText('strong', 'Strong')
    ];
    ...
    switch(input.id) {
        case 'systolic_bp':
            if (value < 70 || value > 250) {
                isValid = false;
                message = getI18nText('systolic_bp', 'Systolic BP should be between 70 and 250 mmHg');
            }
            break;
        case 'diastolic_bp':
            if (value < 40 || value > 150) {
                isValid = false;
                message = getI18nText('diastolic_bp', 'Diastolic BP should be between 40 and 150 mmHg');
            }
            break;
        case 'heart_rate':
            if (value < 30 || value > 200) {
                isValid = false;
                message = getI18nText('heart_rate', 'Heart rate should be between 30 and 200 BPM');
            }
            break;
        case 'blood_sugar':
            if (value < 50 || value > 500) {
                isValid = false;
                message = getI18nText('blood_sugar', 'Blood sugar should be between 50 and 500 mg/dL');
            }
            break;
        case 'cholesterol':
            if (value < 100 || value > 400) {
                isValid = false;
                message = getI18nText('cholesterol', 'Cholesterol should be between 100 and 400 mg/dL');
            }
            break;
    }
...
function setupInteractiveElements() {
    ...
    // Auto-dismiss only flash messages in the top container.
